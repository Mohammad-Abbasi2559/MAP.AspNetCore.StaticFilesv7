        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (checkUrl1.Count == checkUrl2.Count + 1 && checkUrl2.Count > 1)
                    {
                        int index1 = checkUrl1.IndexOf(controllerName);
                        int index2 = checkUrl2.IndexOf(controllerName);

                        string action1 = index1 > -1 && index1 + 1 < checkUrl1.Count ? checkUrl1[index1 + 1] : string.Empty;
                        string action2 = index2 > -1 && index2 + 1 < checkUrl2.Count ? checkUrl2[index2 + 1] : string.Empty;

                        for (int i = 0; i < checkUrl2.Count; i++) if (checkUrl1[i] != checkUrl2[i]) return false;

                        return index1 > -1 && index2 > -1 && checkUrl1.Count - index1 == 3 && action1 == action2;
                    }
                    else if (checkUrl1.Count == checkUrl2.Count + 1 && urlSplit1.Length > 2) return checkUrl1[1] == urlSplit1.LastOrDefault();
                    else if (checkUrl1.Count + 1 == checkUrl2.Count && checkUrl1.Count > 1)
                    {
                        ...
                        return index1 > -1 && index2 > -1 && checkUrl2.Count - index2 == 3 && action1 == action2;
                    }
                    else if (checkUrl2.Count == checkUrl1.Count + 1 && urlSplit2.Length > 2) return checkUrl2[1] == urlSplit2.LastOrDefault();
                    else return false;
                }
        ...
        else
        {
            ...
            else
            {
                if (controllerName == null)
                {
                    ...
                    else if (checkPath1.Count == checkPath2.Count + 1 && pathSplit1.Length > 2) return checkPath1[0] == pathSplit1.LastOrDefault();
                    else if (checkPath1.Count + 1 == checkPath2.Count && checkPath1.Count > 0)
                    {
                        for (int i = 0; i < checkPath1.Count; i++) if (checkPath1[i] != checkPath2[i]) return false;
                    }
                    else if (checkPath2.Count == checkPath1.Count + 1 && pathSplit2.Length > 2) return checkPath2[0] == pathSplit2.LastOrDefault();
                    else return false;
                }
                else
                {
                    ...
                    if (checkPath1.Count == checkPath2.Count + 1 && checkPath2.Count > 0)
                    {
                        int index1 = checkPath1.IndexOf(controllerName);
                        int index2 = checkPath2.IndexOf(controllerName);

                        string action1 = index1 > -1 && index1 + 1 < checkPath1.Count ? checkPath1[index1 + 1] : string.Empty;
                        string action2 = index2 > -1 && index2 + 1 < checkPath2.Count ? checkPath2[index2 + 1] : string.Empty;


                        for (int i = 0; i < checkPath2.Count; i++) if (checkPath1[i] != checkPath2[i]) return false;

                        return index1 > -1 && index2 > -1 && checkPath1.Count - index1 == 3 && action1 == action2;
                    }
                    else if (checkPath1.Count == checkPath2.Count + 1 && pathSplit1.Length > 2) return checkPath1[0] == pathSplit1.LastOrDefault();
                    else if (checkPath1.Count + 1 == checkPath2.Count && checkPath1.Count > 0)
                    {
                        ...
                        return index1 > -1 && index2 > -1 && checkPath2.Count - index2 == 3 && action1 == action2;
                    }
                    else if (checkPath2.Count == checkPath1.Count + 1 && pathSplit2.Length > 2) return checkPath2[0] == pathSplit2.LastOrDefault();
                    else return false;
                }
